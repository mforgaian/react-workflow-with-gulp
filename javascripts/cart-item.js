...
export default class CartItem extends React.Component{
    ...
    componentWillMount(){
        this.recalculteTotal();
    }
    
    increaseQty(){
        this.setState({qty:this.state.qty+1}, this.recalculteTotal);
    }
    
    decreaseQty(){
        newQty = this.state.qty > 0? this.state.qty -1 : 0;
        this.setState({qty:newQty}, this.recalculteTotal);
    }

    recalculteTotal(){
        this.setState({ this.state.qty* this.props.price});
    }
